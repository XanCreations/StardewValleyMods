...
namespace NITVNetBall
{
    public static partial class NetBallMethods
    {
        ...
        private enum NetBallImpactOutcome { None, Caught, Spawned }

        // Keep default unchanged for "none"
        private const string Sfx_NoResult = "dropItemInWater";

        private static void PlayImpactVfxSfx(GameLocation location, Vector2 landingPos, NetBallImpactOutcome outcome)
        {
            if (location is null)
                return;
            ...
            switch (outcome)
            {
                case NetBallImpactOutcome.Caught:
                    Game1.playSound("CatchSFX"); //custom sound for catching creature
                    AddCaughtSplash(location, landingPos);
                    break;

                case NetBallImpactOutcome.Spawned:
                    Game1.playSound("SpawnSFX"); //custom sound for spawning creature
                    AddSpawnedSplash(location, landingPos);
                    break;

                default:
                    Game1.playSound(Sfx_NoResult);
                    AddDefaultSplash(location, landingPos);
                    break;
            }
        }

        private static void AddDefaultSplash(GameLocation location, Vector2 landingPos)
        {
        ...
        // VFX for when a creature has been caught
        private static void AddCaughtSplash(GameLocation location, Vector2 landingPos)
        {
            var splash = new TemporaryAnimatedSprite(
                ModEntry.Instance.CatchVfxTextureName,
                new Rectangle(0, 0, 64, 64),
                200f, 10, 1,
                new Vector2(landingPos.X - 8f, landingPos.Y - 8f),
                false, false,
                (landingPos.Y + 16f) / 10000f,
                0f, Color.White, 1.05f, 0f, 0f, 0f, false
            )
            {
                layerDepth = (landingPos.Y + 16f) / 10000f,
            };
        ...
        // VFX for when a creature has been spawned after throwing the net ball
        private static void AddSpawnedSplash(GameLocation location, Vector2 landingPos)
        {
            var splash1 = new TemporaryAnimatedSprite(
                ModEntry.Instance.SpawnVfxTextureName,
                new Rectangle(0, 0, 64, 64),
                200f, 10, 1,
                new Vector2(landingPos.X - 10f, landingPos.Y - 10f),
                false, false,
                (landingPos.Y + 16f) / 10000f,
                0f, Color.White, 0.9f, 0f, 0f, 0f, false
            )
            {
                layerDepth = (landingPos.Y + 16f) / 10000f,
            };

            var splash2 = new TemporaryAnimatedSprite(
                "TileSheets\\animations",
                new Rectangle(0, 832, 64, 64),
                100f, 10, 1,
                new Vector2(landingPos.X + 2f, landingPos.Y + 2f),
                false, false,
                (landingPos.Y + 16f) / 10000f,
                0f, Color.White, 0.75f, 0f, 0f, 0f, false
            )
            {
                layerDepth = (landingPos.Y + 16f) / 10000f,
            };

            location.temporarySprites.Add(splash1);
            location.temporarySprites.Add(splash2);
        }
        ...
        public static void ThrowNetBall(Farmer who, GameLocation location, Vector2 clickedTile, string usedBallName)
        {
            ...
            void onTick(object s, UpdateTickedEventArgs e)
            {
                ...
                NetBallImpactOutcome outcome = NetBallImpactOutcome.None;

                // Only works on host for real outcomes -> non-host falls back to default impact
                if (Context.IsOnHostComputer)
                {
                    bool caught = CheckForCreatureAndCatch(location, landingPos, who, usedBallName);
                    if (caught)
                    {
                        outcome = NetBallImpactOutcome.Caught;
                    }
                    else
                    {
                        bool spawned = NetBallMethods.TryNetBallSpawnChance(location, landingPos, who, usedBallName);
                        if (spawned)
                            outcome = NetBallImpactOutcome.Spawned;
                    }
                }

                PlayImpactVfxSfx(location, landingPos, outcome);
        ...
        private static bool CheckForCreatureAndCatch(GameLocation location, Vector2 landingPos, Farmer player, string usedBallName)
        {
            ...
            if (string.Equals(usedBallName, "Net Ball=", StringComparison.Ordinal))
                grabRadius *= 2.5f;
