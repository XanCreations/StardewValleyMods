...
namespace NITVNetBall
{
    public static partial class NetBallMethods
    {
        ...
        // Hook so other mods can handle Net Ball spawns after the Net Ball chance and proximity checks pass
        public static Func<GameLocation, Vector2, Farmer, string, Vector2, bool> ExternalSpawnResolver;


        public static bool TryNetBallSpawnChance(GameLocation location, Vector2 landingPos, Farmer who, string usedBallName)
        {
            if (!Context.IsOnHostComputer || location is null)
                return false;

            try
            {
                double chance = string.Equals(usedBallName, "Net Ball=", StringComparison.Ordinal) ? 0.40 : 0.20;
                return MaybeSpawnWaterCreature(location, landingPos, who, usedBallName, chance);
            }
            catch (Exception ex)
            {
        ...
        // Changed from void to bool for VFX/SFX purposes
        private static bool MaybeSpawnWaterCreature(GameLocation location, Vector2 landingPos, Farmer who, string usedBallName, double chance)
        {
            ...
            // choose a random water tile within radius to spawn on (shared by both internal + external spawners)
            ...
            // Give another mod a chance to handle the spawn (e.g. Pokemon SpawnChance rules)
            if (ExternalSpawnResolver != null)
            {
                try
                {
                    if (ExternalSpawnResolver(location, landingPos, who, usedBallName, spawnTile))
                    {
                        Game1.addHUDMessage(new HUDMessage("A water creature appeared!", HUDMessage.newQuest_type));
                        log?.Log("[NetBall] External spawn resolver handled spawn.", LogLevel.Trace);
                        return true;
                    }
                }
                catch (Exception ex)
                {
                    log?.Log("[NetBall] ExternalSpawnResolver crashed: " + ex, LogLevel.Error);
                }
            }

            if (ExternalSpawnResolver == null)
            log?.Log("[NetBall] ExternalSpawnResolver is NULL -> using internal rarity fallback (no SpawnChanceList weighting, shinies blocked).", LogLevel.Debug);
        ...
        private static bool IsBlacklistedKey(string key)
        {
            ...
            // avoid spawning terrarium types
            return key.IndexOf("Terrarium", StringComparison.OrdinalIgnoreCase) >= 0;
        }
