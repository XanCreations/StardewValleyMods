...
namespace NITVPokemonAddon
{
    public class ModEntry : Mod
    {
        ...
        public override void Entry(IModHelper helper)
        {
            ...
            SpawnChance.Initialize(helper, harmony); // Need this to initialize a new spawn chance system
            ...
            // Spiritomb's Relic -> (BC) item interaction (poketeleporter)
            {
                var original = AccessTools.Method(typeof(StardewValley.Object),
                ...
                if (original != null)
                    harmony.Patch(
                        original,
                        prefix: new HarmonyMethod(typeof(Patches), nameof(Patches.PokeTeleporter_CheckForActionPrefix))
                    );
                else
                    this.Monitor.Log("Couldn't find Object.checkForAction(Farmer,bool)", LogLevel.Error);
            }
